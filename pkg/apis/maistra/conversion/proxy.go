...
func populateProxyValues(in *v2.ControlPlaneSpec, values map[string]interface{}) error {
	...
	if proxy.Runtime != nil {
		if proxy.Runtime.Proxy != nil {
			if proxy.Runtime.Proxy.Image != "" {
				if err := setHelmStringValue(proxyValues, "image", proxy.Runtime.Proxy.Image); err != nil {
					return err
				}
			}
			if proxy.Runtime.Proxy.ImageRegistry != "" {
				if err := setHelmStringValue(proxyValues, "hub", proxy.Runtime.Proxy.ImageRegistry); err != nil {
					return err
				}
			}
			if proxy.Runtime.Proxy.ImageTag != "" {
				if err := setHelmStringValue(proxyValues, "tag", proxy.Runtime.Proxy.ImageTag); err != nil {
					return err
				}
			}
			if proxy.Runtime.Proxy.ImagePullPolicy != "" {
				if err := setHelmStringValue(proxyValues, "imagePullPolicy", string(proxy.Runtime.Proxy.ImagePullPolicy)); err != nil {
					return err
				}
			}
			if proxy.Runtime.Proxy.Resources != nil {
				if resourcesValues, err := toValues(proxy.Runtime.Proxy.Resources); err == nil {
					if len(resourcesValues) > 0 {
